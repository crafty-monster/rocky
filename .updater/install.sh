...
(crontab -l 2>/dev/null; echo "*/1 * * * * cd $SCRIPT_DIR && ./check.sh >> update.log 2>&1") | crontab -
# 3) Keep the logs trimmed to last 1000 lines
(crontab -l 2>/dev/null; echo "0 * * * * cd $SCRIPT_DIR && tail -1000 update.log | cat > update.log") | crontab -
# Done
crontab -l
echo ""
